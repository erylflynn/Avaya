...
def extension_list(file):
    if not os.path.isfile(file):
        print("Sorry, vectors.txt does not exist.")
        exit()
    usage = []
    with open(file)as f:
        for line in f:
...
def list_usage(vector):
    ...
    output_final = ''
    global remote_conn
    command = "clist usage vector " + str(vector) + "\n"
...
list_of_ext = (os.path.dirname(__file__) + '/vectors.txt')
...
sheet.cell(row=1, column=1).value = 'Vector'
...
wb.save('List_Usage_vector.xlsx')
