...
def list_usage(extension):
    ...
    output_final = ''
    global remote_conn
    command = "clist usage extension  " + str(extension) + "\n"
...
def list_usage_digit(digits):
    ...
    output_final = ''
    global remote_conn
    command = "clist usage digit-string  " + str(digits) + "\n"
    remote_conn.send(command)
#    time.sleep(.5)
    remote_conn.send(field1)
#    time.sleep(.5)
...
def extension_list(file):
    if not os.path.isfile(file):
        print("Sorry, stations.txt does not exist.")
        exit()
    usage = []
    with open(file)as f:
        for line in f:
...
def create_list(file):
    ...
    for ext in extensions:
        ...
        full_list.append(parse_list_usage(list_usage_digit(ext), ext))
    return full_list
...
list_of_ext = (os.path.dirname(__file__) + '/stations.txt')
...
wb.save('List_Usage_extensions.xlsx')